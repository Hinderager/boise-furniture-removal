...
export function QuickQuoteBar() {
    ...
    try {
      await fetch('/api/ghl/abandoned-lead', {
        ...
        body: JSON.stringify({
          ...
          lastName,
          phone,
          source: 'microsite - furniture removal',
          tags: ['microsite', 'furniture removal'],
          message,
          photoUrls
        })
      })
  ...
  const handleInitialClick = async (e: React.FormEvent) => {
    ...
    try {
      ...
      submitData.append('form_type', 'furniture_removal_microsite')
        ...
        <form onSubmit={isExpanded ? handleSubmit : handleInitialClick}>
          ...
          <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="space-y-3 pt-1">
              ...
              <textarea
                name="message"
                placeholder="Tell us about your furniture removal needs (optional)"
          ...
          >
            {isSubmitting ? (
              ...
              <>
                {isExpanded ? 'Send Quote Request' : 'Get Furniture Removal Quote'}
